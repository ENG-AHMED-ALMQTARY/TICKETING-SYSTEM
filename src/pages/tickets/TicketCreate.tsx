  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto space-y-6">
      <button onClick={() => navigate('/tickets')} className="flex items-center text-slate-400 hover:text-white transition-colors">
        <ArrowLeft className={`w-4 h-4 ${direction === 'rtl' ? 'ml-1 rotate-180' : 'mr-1'}`} /> {t('backToTickets')}
      </button>

      <div>
        <h1 className="text-3xl font-bold text-white font-display">{t('createNewTicket')}</h1>
        <p className="text-slate-400">{t('createTicketSubtitle')}</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-300">{t('issueType')}</label>
              <select 
                value={formData.type}
              ...
              >
                {(Object.values(TicketType) as string[]).map(ticketType => (
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-300">{t('priority')}</label>
              <select 
